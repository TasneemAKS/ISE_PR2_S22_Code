<?php 
  ...
  if(isset($_POST["addhw"])){
    ...
    move_uploaded_file($file_tmp,'./filehw/'.$file_name);
  ...
  if(isset($_GET["delet_hw"])){
    $wid=$_GET["delet_hw"];
    $sql_subject = mysqli_query($conn,"SELECT * FROM hm" );
            while($row = $sql_subject->fetch_assoc()){
    unlink('./filehw/'.$row['file']);
    }
  ...
  if(isset($_POST['update_file'])){
     ...
     if ($up_file_name == null) {
      ...
      unlink('./filehw/'.$_POST['update_old']);
      $up_file_tmp = $_FILES["update_file_f"]["tmp_name"];
      move_uploaded_file($up_file_tmp,'./filehw/'.$up_file_name);
     }
  ...
  if(isset($_GET['download_homework'])){
    ...
    $filepath = './filehw/homework solution/' . $filename;
				    ...
				    <div class="col-md-12">
						...
						<table class="table table-striped table-hover">
						   ...
						   <?php
					          ...
					          $sql_subject = mysqli_query($conn,"SELECT * FROM solution" );
					        while($row = $sql_subject->fetch_assoc()){ 
					        ?>
