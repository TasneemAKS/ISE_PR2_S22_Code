<?php 
  ...
  $sql_name=mysqli_query($conn,"SELECT * FROM techer WHERE wname='$wname' ");
  if(mysqli_num_rows($sql_name) > 0){
    header('Location:teachers.php?q1=1');
    exit();
  }
  ...
  $sql_email=mysqli_query($conn,"SELECT * FROM techer  WHERE wemail='$wemail'");
  if(mysqli_num_rows($sql_email) > 0){
    header('Location:teachers.php?q1=2');
    exit();
  }
  ...
  move_uploaded_file($wfile_tmp,'../teacher/picture/'.$wfile_name);
  $wpassword=$_POST['wpassword'];
  $wpassword2=$_POST['wpassword2'];

  //check if the password is not matching ----------------------------------------------------------------------------
  if($wpassword != $wpassword2 ){
    header('Location:teachers.php?q1=3');
    exit();
  }

  $sql_insert = mysqli_query($conn, "INSERT INTO `techer`(wname, wGender, wdate, wage, wemail, wclass,wbio , wfile, wpassword) VALUES ('$wname','$wGender','$wdate','$wage','$wemail','$wclass' ,'$wbio' ,'$wfile_name','$wpassword') ");
  header('Location:teachers.php');

  }

  //delete teacher ----------------------------------------------------------------------------------------------------
  if(isset($_GET["delet_teacher"])){
    $wid=$_GET["delet_teacher"];
    $sql_subject = mysqli_query($conn,"SELECT * FROM techer WHERE ID = '$wid' " );
		while($row = $sql_subject->fetch_assoc()){
    unlink('../teacher/picture/'.$row['wfile']);
    }
    $sql_delete_referans = mysqli_query($conn ,"DELETE FROM `techer` WHERE ID = '$wid' ");
    header('location:teachers.php');
  }
  ...
  if(isset($_POST['update_file'])){
     ...
     if ($up_file_name == null) {
      ...
      unlink('../teacher/picture/'.$_POST['update_old']);
      $up_file_tmp = $_FILES["update_file_f"]["tmp_name"];
      move_uploaded_file($up_file_tmp,'../teacher/picture/'.$up_file_name);
     }
					   ...
					   <div class="table-title">
					     <div class="row">
						     ...
						     <div class="col-sm-6 p-0 flex justify-content-lg-start justify-content-center">
							    <h2 class="ml-lg-2">إدارة قوائم المدرسين</h2>
							 </div>
					     </div>
					   </div>
					   ...
					   <table class="table table-striped table-hover">
						  ...
						  <tbody>
							<tr>
								...
								<th>
								   <a href="update_teachers.php?edit_teacher=<?php echo $row['ID']; ?>" class="edit" >
								  <i class="material-icons"  title="تعديل">&#xE254;</i></a>

								  <a href="teachers.php?delet_teacher=<?php echo $row['ID']; ?>" class="delete" >
								  <i class="material-icons"  title="حذف">&#xE872;</i>
								  </a>
								<td>
									<a href="teachers.php?edit_file=<?php echo $row['ID']; ?>" >تحديث</a>
								</td>
								</th>
								<td><?php echo $row['wpassword'] ?></td>
								<td><img src="../teacher/picture/<?php echo $row['wfile'] ?>" alt="الصورة الشخصية" width="65"></td>
								<td><?php echo $row['wbio'] ?></td>
						  ...
						  <div class="modal-content">
							<div class="modal-header">
							  ...
							  <form  action="" method="post" enctype="multipart/form-data">
							    ...
							    if($q1 == 3){echo "password not matched";}
							    ?>
							<div class="modal-body">
								...
								<div class="form-group">
									...
									<textarea input type="text" name="wbio" class="form-control" required></textarea>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<input type="password" name="wpassword" class="form-control" required>
								</div>
								<div class="form-group">
									<label>تأكيد كلمة السر</label>
									<input type="password" name="wpassword2" class="form-control" required>
								</div>
							</div>
							<div class="modal-footer">
							  <button type="button" class="btn btn-secondary" data-dismiss="modal">إلغاء</button>
							  <button type="submit" name="addteacher" class="btn btn-success">إضافة</button>
							</div>
						  </div>
						</div>
					  </div>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
		  ...
		  <div class="title-btn"><h5 class="modal-title">تعديل بيانات المدرس</h5></div>
      ...
      <div class="modal-body">
		...
		<div class="form-group">
		    <label>معلومات شخصية</label>
			<textarea input type="text" class="form-control"></textarea>
		</div>
		<div class="form-group">
		    <label>كلمة السر</label>
			<input type="text" class="form-control">
						  ...
						  <div class="modal-content">
							<div class="modal-header">
								...
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								  ...
								  <div class="title-btn"><h5 class="modal-title">حذف مدرس</h5></div>
							</div>
							<div class="modal-body">
							  <p>هل أنت متأكد من حذف هذا المدرس</p>
							</div>
							<div class="modal-footer">
							  <button type="button" class="btn btn-secondary" data-dismiss="modal">إلغاء</button>
