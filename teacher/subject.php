<?php 
  ...
  if(isset($_POST["addsubject"])){
    ...
    move_uploaded_file($file_tmp,'../admin/filesubject/'.$file_name);
  ...
  if(isset($_GET["delet_subject"])){
    ...
    $edit_query = mysqli_query($conn, "SELECT * FROM `subject` WHERE wid = $wid");
	    while($row = $edit_query->fetch_assoc()){
    unlink('../admin/filesubject/'.$row['file']);
  ...
  if(isset($_POST['update_file'])){
     ...
     if ($up_file_name == null) {
      ...
      unlink('../admin/filesubject/'.$_POST['update_old']);
      $up_file_tmp = $_FILES["update_file_f"]["tmp_name"];
      move_uploaded_file($up_file_tmp,'../admin/filesubject/'.$up_file_name);
     }
			     ...
			     <div class="row">
				    <div class="col-md-12">
					   ...
					   <table class="table table-striped table-hover">
						  ...
						  <?php
					        include('../connection.php');
					        $sql_subject = mysqli_query($conn,"SELECT * FROM subject INNER JOIN techer ON techer.wclass = subject.wnameclass WHERE techer.wemail = '$email'" );
